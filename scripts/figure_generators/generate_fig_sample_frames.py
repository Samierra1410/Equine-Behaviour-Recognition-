"""
Generates fig_sample_frames — Figure 5 in the paper.
Three-panel figure: affiliative, neutral, avoidant frames
with real AP-10K ONNX inference, MediaPipe skeleton, and YOLOv8 boxes.

Requires: models/ap10k_hrnet_w32.onnx (run setup_ap10k.py first)

AP-10K keypoint order (official mmpose):
...
def get_ort_session():
    ...
    if not ONNX_PATH.exists():
        ...
        print("Run setup_ap10k.py first to export the ONNX model.")
        return None
...
def get_yolo():
    ...
    if _yolo_model is None:
        ...
        _yolo_model = YOLO(str(p) if p.exists() else 'yolov8n.pt')
    return _yolo_model
...
def run_ap10k(frame, horse_box, padding=0.20):
    ...
    for i, name in enumerate(KP_NAMES):
        ...
        conf = float(hm[i, iy, ix])
        if conf < KP_CONF_THR:
            continue
        kp_dict[name] = (int(ix / hm_w * cw + x1p), int(iy / hm_h * ch + y1p))

    print(f"  AP-10K: {len(kp_dict)}/17 keypoints detected")
        ...
        for c in conns:
            ...
            if l1.visibility > 0.3 and l2.visibility > 0.3:
                cv2.line(img,
                         ...
                         COLORS['mediapipe'], 4, cv2.LINE_AA)
        for idx in [0, 11, 12, 13, 14, 15, 16, 23, 24, 25, 26, 27, 28]:
            lm = mp_lms.landmark[idx]
            if lm.visibility > 0.3:
                pt = (int(lm.x * w), int(lm.y * h))
                cv2.circle(img, pt, 6, COLORS['mediapipe'], -1, cv2.LINE_AA)
                cv2.circle(img, pt, 7, (255, 255, 255), 1, cv2.LINE_AA)

    if horse_kps:
        for i, j in KP_SKELETON:
            a, b = KP_NAMES[i], KP_NAMES[j]
            if a in horse_kps and b in horse_kps:
                cv2.line(img, horse_kps[a], horse_kps[b], COLORS['ap10k'], 3, cv2.LINE_AA)
        for pt in horse_kps.values():
            cv2.circle(img, pt, 6, COLORS['ap10k'], -1, cv2.LINE_AA)
            cv2.circle(img, pt, 7, (255, 255, 255), 1, cv2.LINE_AA)
...
def main():
    print("Generating fig_sample_frames — real AP-10K ONNX inference")
    ...
    for cat in ['affiliative', 'neutral', 'avoidant']:
        ...
        matches = [f for f in VIDEO_DIR.iterdir()
                   if video_name.replace('.MP4', '').replace('.mp4', '') in f.stem]
        if not matches:
        ...
        print(f"  Person: {round(human['conf'], 2) if human else 'not detected'}")
        print(f"  Horse:  {round(horse['conf'], 2) if horse else 'not detected'}")
    ...
    for i, cat in enumerate(cats):
        ...
        axes[i].set_title(titles[cat], fontsize=14, fontweight='bold', pad=10)
        axes[i].axis('off')
    ...
    fig.legend(handles=legend_handles, loc='lower center', ncol=4, fontsize=10,
               frameon=True, fancybox=True, bbox_to_anchor=(0.5, -0.02), edgecolor='#CCC')
